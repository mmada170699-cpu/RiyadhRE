...
ADMIN_ID   = int(os.getenv("ADMIN_USER_ID", "0"))     # your Telegram numeric user ID
...
async def moderate_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text_has_license(text):
        ...
        try:
            ...
            pass
        return
    # Otherwise allowed → keep message
...
# Owner-only: pin / unpin (reply to target message)
...
# ===== Background 1% reminder WITHOUT JobQueue =====
async def _fee_loop(app: Application):
    await asyncio.sleep(30)  # small delay after startup
    while True:
        try:
...
def main():
    application = (
        Application.builder()
        .token(BOT_TOKEN)
        .post_init(_post_init)      # start background reminder loop
        .build()
    )

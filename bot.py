...
GROUP_ID   = int(os.getenv("GROUP_ID", "0"))          # e.g. -4942161299
ADMIN_ID   = int(os.getenv("ADMIN_USER_ID", "0"))     # your Telegram user ID
...
# FAL / license detection and deed (“رقم الصك محدث”)
...
# In-memory offense counter (resets if the bot restarts)
...
def text_has_license(text: str) -> bool:
    ...
    if RE_FAL.search(text) or RE_DEED.search(text):
        return True
    # Accept a bare 9–12 digit string as a last resort (many agents paste just the number)
...
async def moderate_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Run on every message in the group; delete/ban as required."""
    ...
    # 3) Non-Riyadh by location (if a location is shared)
    if msg.location:
    ...
    # 4) License/Deed required (in text or caption)
    ...
    # Otherwise: allowed; do nothing (message stays)

# ===== Commands =====
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_chat.send_message(
        "مرحباً 👋\n"
        "أرسل إعلانك مع **رقم الرخصة (FAL)** أو **رقم الصك محدث** داخل مجموعة الرياض.\n"
        "تذكير: يوجد أتعاب ثابتة قدرها **1%** من قيمة الصفقة."
    )

async def whereami(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    await update.effective_chat.send_message(f"Chat ID: `{chat.id}`\nType: {chat.type}", parse_mode=ParseMode.MARKDOWN)

async def policy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not in_target_group(update):  # only post policy in the group
        return
    await update.effective_chat.send_message(POLICY_TEXT, parse_mode=ParseMode.MARKDOWN)

# Owner-only: pin/unpin (must reply to a message)
...
# Periodic 1% fee reminder
async def fee_reminder(context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.send_message(GROUP_ID, "تذكير ودّي: أتعاب الصفحة ثابتة **1%** من قيمة الصفقة. ✅")
    except Exception:
        pass
...
def main():
    ...
    # Group moderation: text, captions, media, and locations
    group_filter = filters.Chat(GROUP_ID) & filters.ChatType.GROUPS
    application.add_handler(MessageHandler(group_filter & (filters.TEXT | filters.CAPTION), moderate_message))
    application.add_handler(MessageHandler(group_filter & filters.LOCATION, moderate_message))

    # 6-hour reminder
    application.job_queue.run_repeating(fee_reminder, interval=6*60*60, first=30)
